...
<template>
    ...
    <AuthenticatedLayout :title="good.data.title">
        <section v-if="good" class="bg-gray-100 p-4 text-gray-900 dark:bg-gray-900 dark:text-gray-400 sm:p-6 lg:p-8">
            <div class="overflow-hidden bg-white px-4 shadow-sm dark:bg-gray-800 sm:rounded-lg sm:px-6 lg:px-8">
                ...
                <div class="container mx-auto pt-12" v-if="good?.data.properties.length">
                    <dl class="rounded-lg">
                        <div
                            v-for="(property, i) in good.data.properties"
                            :key="property.id"
                            :class="[
                                'flex flex-col px-4 py-5 font-medium sm:flex-row sm:px-6',
                                i % 2 === 0 ? 'bg-gray-100 dark:bg-gray-900' : 'bg-white dark:bg-gray-800',
                                { 'rounded-t-lg': i === 0 },
                                { 'rounded-b-lg': i === good.data.properties.length - 1 }
                            ]"
                        >
                ...
                <p class="font-italic pb-12 pt-6 text-gray-700 dark:text-gray-400">* {{ good.data.warning_description }}</p>
            </div>
        </section>
    </AuthenticatedLayout>
</template>
