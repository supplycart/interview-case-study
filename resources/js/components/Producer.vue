    ...
    <div class="bg-white">
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="relative z-10 flex items-baseline justify-between pt-24 pb-6 border-b border-gray-200">
                <h1 class="text-4xl font-extrabold tracking-tight text-gray-900">Producers List</h1>
                            ...
                            <div class="px-4 py-5 bg-white sm:p-6">
                                <div class="grid grid-cols-6 gap-6">
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="name" class="block text-sm font-medium text-gray-700 required">Producer name</label>
            ...
            <section aria-labelledby="producers-heading" class="pt-6 pb-24">
                <h2 id="producers-heading" class="sr-only">Producers List</h2>
                            ...
                            <div class="bg-white">
                                <div class="max-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
                                    <div class="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
                                        <div v-for="producer in producers" :key="producer.id" class="group relative">
                                            <div class="w-full min-h-80 bg-gray-200 aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:h-80 lg:aspect-none">
                                                <img :src="producer.imagePath" class="w-full h-full object-center object-cover lg:w-full lg:h-full" />
                                            </div>
                                            <div class="mt-4 flex justify-between">
                                                <div>
                                                    <h3 class="text-sm text-gray-700">
                                                        <div @click="openProducer(producer.id)">
                                                            <span aria-hidden="true" class="absolute inset-0" />
                                                            <h3 class="text-lg leading-6 font-bold text-gray-900">{{ producer.name }}</h3>
                                                        </div>
                                                    </h3>
                                                </div>
                                            </div>
                                            <div class="mt-4 flex justify-between">
                                                <p class="text-sm font-medium text-justify text-gray-900">{{ producer.sex }} - {{ producer.dateFormat }}</p>
                                            </div>
                                            <div class="mt-4 flex justify-between">
                                                <p class="text-sm font-medium text-justify text-gray-900">{{ producer.biography }}</p>
                                            </div>
                        ...
                        <div class="w-full relative flex items-center bg-white px-4 pt-14 pb-8 overflow-hidden shadow-2xl sm:px-6 sm:pt-8 md:p-6 lg:p-8">
                            ...
                            <div class="w-full grid grid-cols-1 gap-y-8 gap-x-6 items-start sm:grid-cols-12 lg:gap-x-8">
                                <div class="aspect-w-2 aspect-h-3 rounded-lg bg-gray-100 overflow-hidden sm:col-span-4 lg:col-span-5">
                                    <img :src="producer.imagePath"  class="object-center object-cover" />
                                </div>
                                <div class="sm:col-span-8 lg:col-span-7">
                                    ...
                                    <section aria-labelledby="information-heading" class="mt-2">
                                        <h3 id="information-heading" class="sr-only">Producer Details</h3>

                                        <div class="px-4 py-5 bg-white sm:p-6">
                                            <form @submit.prevent="updateProducer(producer.id)">
                                                <div class="grid grid-cols-6 gap-6">
                                                    <div class="col-span-6 sm:col-span-3">
                                                        <label for="name" class="block text-sm font-medium text-gray-700">Producer name</label>
                                                        <input id="name" type="text" v-model="producer.name" autocomplete="off" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                                    </div>

                                                    <div class="col-span-6 sm:col-span-3">
                                                        <label for="date_of_birth" class="block text-sm font-medium text-gray-700">Date of Birth</label>
                                                        <input id="date_of_birth" type="date" v-model="producer.date_of_birth" autocomplete="off" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                                    </div>

                                                    <div class="col-span-6 sm:col-span-6">
                                                        <label for="biography" class="block text-sm font-medium text-gray-700">Biography</label>
                                                        <textarea id="biography" rows="5" v-model="producer.biography" autocomplete="off" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"></textarea>
                                                    </div>

                                                    <div class="col-span-6 sm:col-span-3">
                                                        <label for="sex" class="block text-sm font-medium text-gray-700">Sex</label>
                                                        <select id="sex" name="sex" v-model="producer.sex" autocomplete="off" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                            <option>Male</option>
                                                            <option>Female</option>
                                                        </select>
                                                    </div>
                                                    ...
                                                    <div class="col-span-12 sm:col-span-6 text-right space-x-4">
                                                        <button @click="deleteProducer(producer.id)" class="mt-6 w-1/3 items-center justify-center text-base font-medium bg-red-600 text-white hover:bg-white hover:text-red-600 font-bold py-2 px-4 rounded inline-flex items-center">Delete</button>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.producers = this.producerData;
        },
        methods: {  
            openProducer(id) {
            ...
            deleteProducer(id) {
                axios.delete('producer/'+ id).then(response => {
            ...
            updateProducer(id) {
                ...
                editData.append('name', this.producer.name)
                editData.append('date_of_birth', this.producer.date_of_birth)
                editData.append('biography', this.producer.biography)
                editData.append('sex', this.producer.sex)
                editData.append('image', this.producer.image ?? '')
                editData.append('imageNew', this.producer.imageNew ?? '')
                editData.append('_method', 'patch')
                axios.post('producer/'+ id, editData).then(response => {
                    if (response.data.status == 'success') {
                        this.open = false
                        this.$toast.success(response.data.message)
            ...
            selectFileUpdate(event) {
                this.producer.imageNew = event.target.files[0];
            }
        },
    }
</script>