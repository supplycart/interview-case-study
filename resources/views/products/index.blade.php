    ...
    <div x-data="{ cartOpen: false , isOpen: false }" class="bg-white">
        ...
        <main class="my-8">
            <div class="container mx-auto px-6">
                <h3 class="text-gray-700 text-2xl font-medium">View Our Product Selections</h3>                                                                       
                <div class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-6">
                    <div class="grid grid-cols-1">
                        <form class="mt-6" method="GET" action={{ URL('products') }}>  
                            @foreach($categories as $category)                                                                                  
                                <div class="flex text-sm w-32">
                                    <input class="mt-1" type="checkbox" name={{ $category->name }} value= {{ $category->id }}>       
                                    <div class="font-bold ml-3">{{ $category->name }}</div>                                
                                </div>  
                            @endforeach                            
                            <div class="flex">
                                <button class="mt-3 text-base  undefined  hover:scale-110 focus:outline-none flex justify-center px-4 py-2 rounded font-bold cursor-pointer
                                    
                                    bg-indigo-700
                                    text-white 
                                    border duration-200 ease-in-out 
                                    border-teal-600 transition">Search</button>
                                <button class="mt-3 ml-3 text-base  undefined  hover:scale-110 focus:outline-none flex justify-center px-4 py-2 rounded font-bold cursor-pointer 
                                    hover:bg-teal-700 hover:text-teal-100 
                                    bg-red-700
                                    text-white 
                                    border duration-200 ease-in-out 
                                    border-teal-600 transition">Reset</button>
                            </div>
                        </form>        
                    </div>                            
                    @foreach($products as $product) 
                        <form class="mt-6" method="POST" action={{ URL('user_orders') }}>  
                            <div class="w-full max-w-sm mx-auto rounded-md shadow-md overflow-hidden">
                                ...
                                <div class="px-5 py-3">
                                    ...
                                    <span class="text-gray-500 mt-2">${{ $product->getCurrentPrice() }}</span>
                                </div>
