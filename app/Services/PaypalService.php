...
class PaypalService
{
    ...
    {
        if (config('services.paypal.paypal_client_id') == '' || config('services.paypal.paypal_secret_id') == '') {
            return redirect()->back()->with('error', 'No PayPal settings found.');
        }

        $this->payPal = new ApiContext(
            new OAuthTokenCredential(
                config('services.paypal.paypal_client_id'),
                config('services.paypal.paypal_secret_id')
            )
        );
        dd(config('services.paypal.paypal_client_id'));
    ...
    public function completePayment($paymentId, $payerId)
    {
        ...
        if ($result->state === 'approved') {
            ...
            return ['salesId' => $saleId, 'invoiceId' => $invoiceId];
        } else {
